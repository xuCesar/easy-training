...
学员报名、账单生成、收款流水、续费、转课、退费、班级排课、考勤和课消已形成基础 PostgreSQL 业务闭环，并按机构、角色和校区范围处理。教务支持每周周期规则的可编辑冲突预览、原子生成、未来课次批量调整与停用规则后的“保留或批量取消”分支；教师账号可显式绑定成员并使用个人课表保存点名草稿或课后结课。工作台仅聚合已经产生的真实业务数据，不使用 Mock 数据补齐空结果。

当前仍未覆盖独立报名入口、补课、班级级停复课、隔周/节假日例外、完整业务时间线、手工开单与冲正、在线支付自动对账、外部通知、家长端、原生教师移动端，以及生产告警和真实灾备演练。
