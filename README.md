...
学员报名、账单生成、收款流水、续费、转课、退费、班级排课、考勤和课消已形成基础 PostgreSQL 业务闭环，并按机构、角色和校区范围处理。工作台仅聚合已经产生的真实业务数据，不使用 Mock 数据补齐空结果。

当前仍未覆盖独立报名入口、周期排课与批量课次生成、调课/补课、完整业务时间线、手工开单与冲正、在线支付自动对账、外部通知、家长或教师端，以及生产告警和真实灾备演练。
