...
function FilterSelect({
	...
	items: Array<{ value: string; label: string } | { id: string; name: string }>;
}) {
	return (
		<Select value={value} onValueChange={(next) => next && onValueChange(next)}>
			...
			<SelectContent>
				<SelectGroup>
					{items.map((item) => {
						const option =
							"value" in item ? item : { value: item.id, label: item.name };
						return (
...
function TextField({
	...
	name: "name" | "phone" | "source";
	label: string;
	values: LeadFormValues;
	setValues: Dispatch<SetStateAction<LeadFormValues>>;
	error?: string;
	type?: "text" | "tel";
	required?: boolean;
	minLength?: number;
...
function getFilterLabel(
	...
	const item = items.find(
		(candidate) =>
			("value" in candidate ? candidate.value : candidate.id) === value,
	);
