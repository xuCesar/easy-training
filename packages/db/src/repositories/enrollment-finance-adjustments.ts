...
export async function createRefundRecord(input: {
	...
	amountInCents: number;
	refundedAt: Date;
	method: (typeof refund.$inferInsert)["method"];
	reason: string;
	requestId: string;
}): Promise<{ refund: RefundRecord }> {
	assertPositiveInteger(input.amountInCents);
	assertValidTime(input.refundedAt);
	...
	try {
		const refundId = await db.transaction(async (tx) => {
			...
			const [existing] = await tx
				...
				.where(
					and(
						...
						eq(refund.requestId, input.requestId),
					),
				)
				.limit(1);
			if (existing) {
			...
			if (invoiceRecord.status !== "paid")
				throw new EnrollmentFinanceAdjustmentError("INVOICE_NOT_REFUNDABLE");
			...
			if ((refundTotal?.value ?? 0) + input.amountInCents > paidAmountInCents)
				throw new EnrollmentFinanceAdjustmentError("REFUND_EXCEEDS_PAID");
			const [operator] = await tx
				.select({ name: user.name })
				.from(user)
				.where(eq(user.id, input.operatorUserId))
